...
// Fleet vessel — https://github.com/Lucineer/tutor-ai
...
const HTML = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Tutor</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#0a0a0f;color:#e2e8f0;min-height:100vh}a{color:#a855f7;text-decoration:none}.hero{min-height:80vh;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;padding:4rem 2rem}.hero h1{font-size:clamp(2rem,5vw,3.5rem);color:#a855f7;margin-bottom:1rem}.hero p{color:#94a3b8;max-width:500px;line-height:1.6;margin-bottom:2rem}.btn{display:inline-block;padding:.75rem 2rem;border-radius:8px;font-weight:600;transition:all .2s}.btn-primary{background:#a855f7;color:#0a0a0f}.btn-primary:hover{opacity:.85}.links{display:flex;gap:1rem;flex-wrap:wrap;justify-content:center;margin-top:1rem}.card{background:#1a1a2e;border:1px solid #2a2a4a;border-radius:12px;padding:2rem;margin:1rem;max-width:400px}.cards{display:flex;flex-wrap:wrap;justify-content:center;gap:1rem;padding:2rem}.section{padding:4rem 2rem;text-align:center}footer{padding:2rem;text-align:center;color:#475569;font-size:.85rem}</style></head><body><div class="hero"><h1>AI Tutor</h1><p>Personalized learning, adaptive teaching. Part of the Lucineer fleet.</p><div class="links"><a href="/setup" class="btn btn-primary">Setup</a><a href="https://github.com/Lucineer/tutor-ai" class="btn" style="border:1px solid #2a2a4a">GitHub</a><a href="https://github.com/Lucineer/capitaine" class="btn" style="border:1px solid #2a2a4a">Fleet</a></div></div><div class="section"><h2 style="color:#a855f7;margin-bottom:1rem">Fleet Standard Routes</h2><div class="cards"><div class="card"><h3>/health</h3><p style="color:#94a3b8;margin-top:.5rem">Health check endpoint</p></div><div class="card"><h3>/setup</h3><p style="color:#94a3b8;margin-top:.5rem">BYOK provider setup</p></div><div class="card"><h3>/api/chat</h3><p style="color:#94a3b8;margin-top:.5rem">Chat with BYOK model</p></div><div class="card"><h3>/api/seed</h3><p style="color:#94a3b8;margin-top:.5rem">Seed data management</p></div></div></div><footer>Superinstance & Lucineer (DiGennaro et al.) &middot; <a href="https://github.com/orgs/Lucineer/repositories">Fleet</a></footer><div style="text-align:center;padding:24px;color:#475569;font-size:.75rem"><a href="https://the-fleet.casey-digennaro.workers.dev" style="color:#64748b">⚓ The Fleet</a> · <a href="https://cocapn.ai" style="color:#64748b">Cocapn</a></div></body></html>';

const SETUP_HTML = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Setup</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#0a0a0f;color:#e2e8f0;padding:2rem;max-width:600px;margin:0 auto}h1{color:#a855f7;margin-bottom:1rem}a{color:#a855f7}p{color:#94a3b8;line-height:1.6;margin-bottom:1rem}code{background:#1a1a2e;padding:.2rem .5rem;border-radius:4px;font-size:.9rem}ol{color:#94a3b8;padding-left:1.5rem;line-height:2}li a{color:#a855f7}</style></head><body><h1>Setup AI Tutor</h1><p>Add your LLM API key via Cloudflare Secrets Store:</p><ol><li>Go to <a href="https://dash.cloudflare.com">Cloudflare Dashboard</a></li><li>Navigate to Workers &rarr; tutor-ai &rarr; Settings &rarr; Variables</li><li>Add one or more secrets:<br><code>DEEPSEEK_API_KEY</code> — DeepSeek (recommended)<br><code>MOONSHOT_API_KEY</code> — Moonshot/Kimi<br><code>DEEPINFRA_API_KEY</code> — DeepInfra<br><code>SILICONFLOW_API_KEY</code> — SiliconFlow</li><li>Visit <a href="/health">/health</a> to verify</li></ol><p style="margin-top:2rem"><a href="/">Back to home</a></p><div style="text-align:center;padding:24px;color:#475569;font-size:.75rem"><a href="https://the-fleet.casey-digennaro.workers.dev" style="color:#64748b">⚓ The Fleet</a> · <a href="https://cocapn.ai" style="color:#64748b">Cocapn</a></div></body></html>';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === '/health') return new Response(JSON.stringify({ status: 'ok', vessel: 'tutor-ai', timestamp: Date.now() }), { headers: j });
    ...
    if (path === '/api/chat' && request.method === 'POST') {
      try {
        ...
        const messages = [{ role: 'system', content: 'You are AI Tutor. Personalized learning, adaptive teaching. Be helpful and concise.' }, ...(body.messages || [{ role: 'user', content: body.message || 'Hello' }])];
