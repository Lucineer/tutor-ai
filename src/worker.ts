...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ── Health ──
    if (url.pathname === '/api/efficiency') return new Response(JSON.stringify({ totalCached: 0, totalHits: 0, cacheHitRate: 0, tokensSaved: 0, repo: 'tutor-ai', timestamp: Date.now() }), { headers: { ...cors, 'Content-Type': 'application/json' } });
    if (url.pathname === '/health') return new Response(JSON.stringify({ status: 'ok', vessel: 'tutor-ai', ts: Date.now() }), { headers: { ...cors, 'Content-Type': 'application/json' } });
...
<p style="margin-top:2rem;color:#999;font-size:.8rem"><a href="https://cocapn.ai">cocapn.ai</a> · <a href="https://github.com/Lucineer/tutor-ai">GitHub</a></p>
...
<header style="padding:.6rem 1rem;text-align:center;border-bottom:1px solid #eee;color:${ACCENT};font-weight:700;font-size:.85rem;background:#fff">⚓ tutor-ai</header>
